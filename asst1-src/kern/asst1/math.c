 ...
 * **********************************************************************
 */
//Semaphore that ensures mutual exclusion
...
static void adder(void * unusedpointer, unsigned long addernumber)
{
        ...
        while (flag) {
                ...
                //Ensure mutex for critical region 
                P(mutex);
                a = counter;
                if (a < NADDS) {
...
int maths (int data1, char **data2)
{
         ...
         * ********************************************************************
         */
        //Create semaphore, return error if cant be created
        mutex = sem_create("mutex",1);
        if (mutex == NULL) {
                panic("maths: sem create mutex failed");
        }
